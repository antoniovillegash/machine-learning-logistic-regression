#preprocesamiento de datos
#librerias
import numpy as np
import matplotlib.pyplot as plt
import pandas as pd
#importando el set de datos
#read_csv es una funcion de pandas para usar datasets
dataset = pd.read_csv('Social_Network_Ads.csv')
#creamos un arreglo con las columnas que necesitamos o sea todas menos la ultima, usamos rangos
X = dataset.iloc[:,[2,3]]
#ahora necesitamos el arreglo de las varibles dependientes
Y = dataset.iloc[:,4]
#dividimos el dataset en training set y test set
from sklearn.model_selection import train_test_split
X_train, X_test, Y_train, Y_test =train_test_split(X,Y,test_size=0.25,random_state=0)

#escalando las variables
from sklearn.preprocessing import StandardScaler
sc_X=StandardScaler()
X_train=sc_X.fit_transform(X_train)
X_test=sc_X.transform(X_test)

#regresion logistica para el set de entrenamiento
from sklearn.linear_model import LogisticRegression
classifier = LogisticRegression(random_state = 0)
classifier.fit(X_train,Y_train)

#predecir los resultados del test set
y_pred = classifier.predict(X_test)

#creando la matriz de confusion
from sklearn.metrics import confusion_matrix
cm = confusion_matrix(Y_test, y_pred)

#visualizando el set de entrenamiento
from matplotlib.colors import ListedColormap
X_set, y_set = X_test, Y_test
X1, X2 = np.meshgrid(np.arange(start = X_set[:, 0].min() - 1, stop = X_set[:, 0].max() + 1, step = 0.01),
                     np.arange(start = X_set[:, 1].min() - 1, stop = X_set[:, 1].max() + 1, step = 0.01))
plt.contourf(X1, X2, classifier.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
             alpha = 0.75, cmap = ListedColormap(('red', 'green')))
plt.xlim(X1.min(), X1.max())
plt.ylim(X2.min(), X2.max())
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('regresión logistica (test)')
plt.xlabel('edad')
plt.ylabel('salario')
plt.legend()
plt.show()